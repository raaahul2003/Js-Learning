...
console.log("");
console.log("Costly product name:",products.reduce((h,a) => h[2]>a[2]?h:a,0)[1]);

// 5. dispaly product and stock , stock in descending order
console.log("");
console.log("Product and Stock");
console.log("-----------------");
products.sort((a,b) => b[3]-a[3]).forEach(c => console.log(c[1],c[3]))

// 6. product having maximum available stock
console.log("");
console.log("Max stock product name:",products.reduce((h,a) => h[3]>a[3]?h:a,0)[1]);

// 7. is there is any product can be purchased by Rs 10
console.log("");
console.log("Product can be purchased by Rs 10");
const aff = products.filter(c => c[2]<=10)
if(aff.length==0){
    console.log("No Product");
}else{
    aff.forEach(c=> console.log(c[1]))
}



// 8. print all product in the range of 10 to 30
console.log("");
